...
data = []
for line in open(".//output//output1.txt","r"):
...
for i in range(mn):
    for j in range(itn):
        ...
        plt.savefig(".//output//fig_"+mode[i]+"_"+input_type[j]+".png",bbox_inches="tight")
        plt.show()
        ...
        df2.index=parse
        df2.columns=size
        df1.to_csv(".//output//mean_"+mode[i]+"_"+input_type[j]+".csv")
        df2.to_csv(".//output//std_"+mode[i]+"_"+input_type[j]+".csv")
...
for i in range(mn):
    for j in range(itn):
        ...
        plt.savefig(".//output//fig_"+mode[i]+"_"+input_type[j]+"_1e5.png",bbox_inches="tight")
        plt.show()

#using Parsersの所要時間
data = []
for line in open(".//output//output2.txt","r"):
    data.append(float(line[:-1].split()[0]))
